        ...
        if (args[0] === HELP || args[0] === HELP_SHORT || args.length < 1) {
            ...
            try {
                ...
                if (numResults > 0) {
                    ...
                    message.channel.send(`TT! I coudln't find any reactions containing \`${searchKey}\`!`);
                }
            } catch (err) {
                console.error(err);
                message.channel.send(`TT! I coudln't find any reactions containing \`${searchKey}\` because: ${err}`);
            }
        }
        return;
    },
